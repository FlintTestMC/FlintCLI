...
use super::{block, recorder, TestExecutor, COMMAND_DELAY_MS, DEFAULT_TESTS_DIR, TEST_RESULT_DELAY_MS};
...
impl TestExecutor {
    ...
    pub(super) async fn handle_help(&mut self) -> Result<()> {
        ...
        self.bot.send_command("say !search <pattern> - Search tests by name").await?;
        self.bot.send_command("say !run <test_name> [step] - Run a specific test").await?;
        self.bot.send_command("say !run-all - Run all tests").await?;
        self.bot.send_command("say !run-tags <tag1,tag2> - Run tests with tags").await?;
        self.bot.send_command("say !list - List all tests").await?;
        self.bot.send_command("say !reload - Reload test files").await?;
        self.bot.send_command("say Recorder: !record <name>, !tick/!next, !save, !cancel").await?;
        self.bot.send_command("say Recorder actions: !assert <x> <y> <z>, !assert_changes").await?;
        self.bot.send_command("say !stop - Exit interactive mode").await?;
        Ok(())
    }

    pub(super) async fn handle_list(&mut self, all_test_files: &[std::path::PathBuf]) -> Result<()> {
        self.bot.send_command(&format!("say Found {} tests:", all_test_files.len())).await?;
        for test_file in all_test_files {
            if let Ok(test) = TestSpec::from_file(test_file) {
                ...
                self.bot.send_command(&format!("say - {}{}", test.name, tags)).await?;
    ...
    pub(super) async fn handle_search(&mut self, all_test_files: &[std::path::PathBuf], pattern: &str) -> Result<()> {
        ...
        if found == 0 {
            self.bot.send_command(&format!("say No tests matching '{}'", pattern)).await?;
        } else {
            self.bot.send_command(&format!("say Found {} matching tests", found)).await?;
        }
        Ok(())
    }
    ...
    pub(super) async fn handle_run(
        ...
        if let Some(test) = found_test {
            if step_mode {
                self.bot.send_command(&format!("say Running test: {} (step mode - type 's' or 'c')", test.name)).await?;
            } else {
                self.bot.send_command(&format!("say Running test: {}", test.name)).await?;
            }
            ...
            self.bot.send_command(&format!("say Test '{}' not found", test_name)).await?;
        }
        Ok(())
    }

    pub(super) async fn handle_run_all(&mut self, all_test_files: &[std::path::PathBuf]) -> Result<()> {
        self.bot.send_command(&format!("say Running all {} tests...", all_test_files.len())).await?;
    ...
    pub(super) async fn handle_run_tags(&mut self, test_loader: &TestLoader, tags: &[String]) -> Result<()> {
        let test_files = test_loader.collect_by_tags(tags)?;

        if test_files.is_empty() {
            self.bot.send_command(&format!("say No tests found with tags: {:?}", tags)).await?;
            return Ok(());
        }

        self.bot.send_command(&format!("say Running {} tests with tags {:?}...", test_files.len(), tags)).await?;
    ...
    pub(super) async fn handle_record_start(
        ...
        if self.recorder.is_some() {
            self.bot.send_command("say Recording already in progress. Use !save or !cancel first.").await?;
            return Ok(());
        }
        ...
        let scan_center = match self.bot.get_position() {
            Ok(pos) => pos,
            Err(_) => {
                self.bot.send_command("say Warning: Could not get bot position, using spawn").await?;
                [0, 64, 0]
            }
        };
        ...
        let initial_blocks = self.scan_blocks_around(scan_center, recorder_state.scan_radius).await?;
        ...
        self.bot.send_command(&format!("say Recording started: {}", test_name)).await?;
        self.bot.send_command("say Time frozen. Block changes will be detected automatically!").await?;
        self.bot.send_command("say Commands: !assert (add check), !tick (step game tick), !save, !cancel").await?;
    ...
    pub(super) async fn handle_record_tick(&mut self) -> Result<()> {
        ...
        if self.recorder.is_none() {
            self.bot.send_command("say No recording in progress. Use !record <name> to start.").await?;
            return Ok(());
        }
        ...
        self.bot.send_command(&format!("say Stepped game tick, now recording tick {} (was {})", new_tick, current_tick)).await?;
    ...
    pub(super) async fn handle_record_assert_changes(&mut self) -> Result<()> {
        let Some(recorder) = self.require_recorder() else {
            self.bot.send_command("say No recording in progress.").await?;
            return Ok(());
        };
        
        let count = recorder.convert_actions_to_asserts();
        self.bot.send_command(&format!("say Converted {} actions to assertions for this tick.", count)).await?;
        Ok(())
    }
    ...
    pub(super) async fn handle_record_save(&mut self) -> Result<bool> {
        ...
        if recorder.timeline.is_empty() {
            self.bot.send_command("say Warning: No actions recorded! Test will be empty.").await?;
        }
        ...
        match recorder.save() {
            Ok(path) => {
                ...
                self.bot.send_command(&format!("say To execute: !run {}", recorder.test_name)).await?;
                println!("To execute this test locally:\ncargo run -- --server localhost:25565 {}", recorder.test_name);
            }
            Err(e) => {
                self.bot.send_command(&format!("say Failed to save test: {}", e)).await?;
    ...
    pub(super) async fn handle_record_snapshot(&mut self) -> Result<()> {
        ...
        self.bot.send_command("say Scanning for block changes...").await?;
        ...
        for (pos, _prev_block) in &initial_snapshot {
            ...
            if current.map(|b| b.to_lowercase().contains("air")).unwrap_or(true) {
        ...
        self.bot.send_command(&format!("say Found {} block changes", changes)).await?;
        Ok(())
    }
